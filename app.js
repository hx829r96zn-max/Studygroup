...
function useOfflineMode(){window._offlineMode=true;var ls=document.getElementById('loginScreen');if(ls)ls.style.display='none';var nav=document.querySelector('nav');if(nav)nav.style.display='flex';setTimeout(function(){renderProfUI();renderHBet();buildStreak();buildHm('hmHome');},200);setTimeout(function(){startFbSync();resumeFbSubscriptions();checkJoinParam();},500);toast('오프라인 모드로 시작합니다');_restoreLastTab();}
...
function loadUserDataFromFirebase(uid){if(!window._fbReady)return;window._fbGet('userdata/'+uid).then(function(snap){if(!snap)return;var data=snap.val?snap.val():null;if(!data)return;var todayStr=new Date().toDateString();if(data.prof&&data.prof.name){prof=data.prof;svp();}if(data.subjs&&data.subjs.length){subjs=data.subjs;sv();}if(data.ctr&&data.ctr.rules){ctr=data.ctr;svc();}if(data.frds&&data.frds.length){frds=data.frds;svf();}if(data.bets&&data.bets.length){bets=data.bets;svb();}if(data.cfg){cfg=data.cfg;svcfg();applyAllTheme();}if(data.todos){todos=data.todos;svTodos();}if(data.planCells&&data.planCells.date===today()&&data.planCells.cells){planCells=data.planCells.cells;svPlanLocal();var _tp=document.getElementById('pg-timer');if(_tp&&_tp.classList.contains('on')){renderTL();}}if(data.todayDate===todayStr&&data.todaySess&&data.todaySess.length){var allStarts={};sess.forEach(function(s){allStarts[s.start]=true;});data.todaySess.forEach(function(s){if(!allStarts[s.start]){sess.push(s);allStarts[s.start]=true;}});localStorage.setItem('sg_ss',JSON.stringify(sess));}else if(data.todaySess&&data.todaySess.length&&!sess.length){sess=data.todaySess;localStorage.setItem('sg_ss',JSON.stringify(sess));}renderProfUI();updateHome();renderHBet();buildStreak();buildHm('hmHome');if(data.prof&&data.prof.name)toast('🔄 기록 복원됨!');}).catch(function(e){console.warn('복원 실패:',e);});}
function saveUserDataToFirebase(){if(!window._fbReady||!window._fbUser)return;var uid=window._fbUser.uid;if(uid.indexOf('offline_')===0)return;var todaySess=sess.filter(function(s){return studyDayOf(new Date(s.start))===today();});return window._fbSet('userdata/'+uid,{prof:prof,subjs:subjs,ctr:ctr,frds:frds,bets:bets,cfg:cfg,todaySess:todaySess,todayDate:today(),planCells:{date:today(),cells:planCells},todos:todos,lastSaved:Date.now()});}
...
// SAVE
function sv(){localStorage.setItem('sg_s',JSON.stringify(subjs));localStorage.setItem('sg_ss',JSON.stringify(sess));}
...
function togglePomo(){
  if(pRun){clearInterval(pInt);pRun=false;_pomoStartAt=null;var pb=document.getElementById('pBtn');if(pb){pb.textContent='▶ 계속';pb.classList.remove('running');}svPomoState();if(aId&&aId===pomoSubjId)stopSubj();renderHomeSubjGrid();}
  else{if(!pomoSubjId){toast('먼저 과목을 선택하세요');return;}pRun=true;if(pCur<=0)pCur=pFocus?pSec:pBrk;var startCur=pCur;_pomoStartAt=Date.now();var pb2=document.getElementById('pBtn');if(pb2){pb2.textContent='⏸ 멈춤';pb2.classList.add('running');}if(pFocus&&aId!==pomoSubjId)startSubj(pomoSubjId);renderHomeSubjGrid();svPomoState();
...
function toggleSubj(){if(!selId)return;if(aId===selId)stopSubj();else startSubj(selId);}
...
function resumeFbSubscriptions(){if(!window._fbReady){setTimeout(resumeFbSubscriptions,1000);return;}JSON.parse(localStorage.getItem('sg_connected')||'[]').forEach(function(c){subscribeToFriend(c.code);});frds.forEach(function(f){if(f.shareCode)subscribeToFriend(f.shareCode);});}
...
window.addEventListener('beforeunload',function(){if(aId&&aStart)sess.push({subjectId:aId,color:(subjs.find(function(s){return s.id===aId;})||{}).color||'#888',start:aStart,end:Date.now()});localStorage.setItem('sg_s',JSON.stringify(subjs));localStorage.setItem('sg_ss',JSON.stringify(sess));localStorage.setItem('sg_c',JSON.stringify(ctr));localStorage.setItem('sg_f',JSON.stringify(frds));localStorage.setItem('sg_b',JSON.stringify(bets));localStorage.setItem('sg_p',JSON.stringify(prof));localStorage.setItem('sg_cfg',JSON.stringify(cfg));try{saveUserDataToFirebase();}catch(e){}});

setInterval(function(){localStorage.setItem('sg_ss',JSON.stringify(sess));saveUserDataToFirebase();},30000);
