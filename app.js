...
function useOfflineMode(silent){window._offlineMode=true;localStorage.setItem('sg_authchoice','offline');document.documentElement.removeAttribute('data-new-user');var ls=document.getElementById('loginScreen');if(ls)ls.style.display='none';var nav=document.querySelector('nav');if(nav)nav.style.display='flex';setTimeout(function(){renderProfUI();renderHBet();buildStreak();buildHm('hmHome');},200);setTimeout(function(){startFbSync();resumeFbSubscriptions();checkJoinParam();},500);_restoreLastTab();}
...
var ROW_H=20,NCOLS=6,COL_MINS=10,AXIS_W=22,GRID_START_HOUR=5;
...
function go(id){
  ...
  var map={home:0,timer:1,room:2,stats:3,settings:4};
...
// HOME
function syncAllScreens(){var t=getTSecs();updateHome();var tt=document.getElementById('timerTotal');if(tt)tt.textContent=fmtHM(t);renderSL();renderTL();checkStreakPoints();checkGoalBonus();if(Math.floor(Date.now()/1000)%30===0)if(frds.some(function(f){return f.fbLiveTimer&&f.fbLiveTimer.active;}))renderConnectedMembers();}
function updateHome(){
  var t=getTSecs(),h=Math.floor(t/3600),m=Math.floor((t%3600)/60),sec=Math.floor(t%60);
  var ht=document.getElementById('homeTotal');
  if(ht)ht.innerHTML=h+'<span>h</span>'+String(m).padStart(2,'0')+'<span>m</span>'+String(sec).padStart(2,'0')+'<span>s</span>';
  var pct=Math.min(100,Math.round(t/goal()*100));
  var hb=document.getElementById('homeBar');if(hb)hb.style.width=pct+'%';
  var hp=document.getElementById('homePct');if(hp)hp.textContent=pct+'%';
  var gl=document.getElementById('goalLbl');
  if(gl){var gh=Math.floor(goal()/3600),gm=Math.floor((goal()%3600)/60);gl.textContent='목표 '+gh+'h'+(gm>0?' '+gm+'m':'');}
  var bars=document.getElementById('homeSubjBars');if(!bars)return;
  bars.innerHTML='';var mx=1;
  subjs.forEach(function(s){var sc=getSecs(s.id);if(sc>mx)mx=sc;});
  subjs.forEach(function(sub){
    var st=getSecs(sub.id);if(!st)return;
    var sh=Math.floor(st/3600),sm=Math.floor((st%3600)/60),ss2=st%60;
    var ts=sh>0?sh+':'+String(sm).padStart(2,'0')+':'+String(ss2).padStart(2,'0'):sm+':'+String(ss2).padStart(2,'0');
    var r=document.createElement('div');
    r.style.cssText='display:flex;align-items:center;gap:6px;margin-bottom:5px;';
    r.innerHTML='<div style="width:34px;font-size:.68rem;font-weight:600;color:'+sub.color+';overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+sub.name+'</div>'+
      '<div style="flex:1;height:5px;background:rgba(255,255,255,.1);border-radius:3px;overflow:hidden;">'+
        '<div style="height:100%;border-radius:3px;background:'+sub.color+';width:'+Math.round(st/mx*100)+'%;"></div>'+
      '</div>'+
      '<div style="font-family:monospace;font-size:.63rem;color:rgba(255,255,255,.4);width:44px;text-align:right;">'+ts+'</div>';
    bars.appendChild(r);
  });
}
...
function renderTH(){
  ...
  var td=document.getElementById('timerDay');if(td)td.textContent=(now.getMonth()+1)+'.'+now.getDate()+' ('+days[now.getDay()]+')';
  var tt=document.getElementById('timerTotal');if(tt)tt.textContent=fmtHMS(getTSecs());
  var ls=document.getElementById('dvLiveSubj');
  if(ls){if(aId&&aStart){var sub=subjs.find(function(s){return s.id===aId;});ls.textContent=sub?'● '+sub.name+' '+f3(Math.floor((Date.now()-aStart)/1000)):'';ls.style.color=sub?sub.color:'#888';}else ls.textContent='';}
  _ptUpdateBar();
}

var _ptSelId=null;
function _ptUpdateBar(){
  var lbl=document.getElementById('ptBarLbl'),btn=document.getElementById('ptBarBtn');if(!lbl||!btn)return;
  if(_ptSelId){var sub=subjs.find(function(s){return s.id===_ptSelId;});if(sub){var r=(aId===_ptSelId);lbl.textContent='● '+sub.name;lbl.className='pt-bar-lbl sel';lbl.style.color=sub.color;btn.textContent=r?'■ 정지':'▶ 시작';btn.className='pt-bar-btn'+(r?' running':'');return;}}
  lbl.textContent='과목 선택';lbl.className='pt-bar-lbl';lbl.style.color='';btn.textContent='▶ 시작';btn.className='pt-bar-btn';
}
function ptToggleSubj(){if(!_ptSelId){toast('과목 칸을 탭하세요');return;}if(aId===_ptSelId)stopSubj();else startSubj(_ptSelId);}

function _ptKey(){return 'sg_ptrows_'+today();}
function _ptLoad(){try{return JSON.parse(localStorage.getItem(_ptKey())||'null');}catch(e){return null;}}
function _ptSave(rows){localStorage.setItem(_ptKey(),JSON.stringify(rows));}
function _ptGetRows(){var saved=_ptLoad();if(saved&&saved.length)return saved;var rows=[];for(var i=0;i<16;i++)rows.push({subjId:null,text:'',status:null});return rows;}

function _ptPickSubj(onSelect){
  var old=document.getElementById('_ptPicker');if(old)old.remove();
  var over=document.createElement('div');over.id='_ptPicker';
  over.style.cssText='position:fixed;inset:0;z-index:200;background:rgba(0,0,0,.4);display:flex;align-items:flex-end;';
  var sheet=document.createElement('div');
  sheet.style.cssText='width:100%;background:#f5f0e8;border-radius:18px 18px 0 0;padding:14px 14px calc(20px + env(safe-area-inset-bottom,0));max-height:60vh;overflow-y:auto;';
  var title=document.createElement('div');title.style.cssText='font-size:.78rem;font-weight:800;color:#1a1a1a;margin-bottom:10px;';title.textContent='과목 선택';sheet.appendChild(title);
  var none=document.createElement('div');none.style.cssText='padding:9px 8px;font-size:.76rem;color:#aaa;cursor:pointer;border-radius:8px;margin-bottom:3px;';none.textContent='없음';
  none.onclick=function(){over.remove();onSelect(null);};sheet.appendChild(none);
  subjs.forEach(function(sub){
    var sc=getSecs(sub.id);var isR=(aId===sub.id);
    var btn=document.createElement('div');
    btn.style.cssText='display:flex;align-items:center;gap:10px;padding:10px 8px;border-radius:9px;cursor:pointer;margin-bottom:3px;';
    btn.innerHTML='<div style="width:9px;height:9px;border-radius:50%;background:'+sub.color+';flex-shrink:0'+(isR?';box-shadow:0 0 0 2px rgba(185,28,28,.35)':'')+';"></div><div style="font-size:.8rem;font-weight:700;color:#1a1a1a;flex:1;">'+escapeHtml(sub.name)+'</div>'+(sc>0?'<div style="font-size:.64rem;font-family:monospace;color:#888;">'+fmtSh(sc)+'</div>':'');
    btn.onclick=function(){over.remove();onSelect(sub.id);};
...
function ptAddRows(n){var rows=_ptGetRows();for(var i=0;i<n;i++)rows.push({subjId:null,text:'',status:null});_ptSave(rows);renderSL();}
  ...
  rows.forEach(function(row,i){
    ...
    if(isSubjRow){
      ...
      (function(idx){
        subjCell.onclick=function(e){
          e.stopPropagation();
          if(planMode){
            // 계획 모드: 이 과목 색으로 바로 선택
            _ptSelId=sub.id;selId=sub.id;_ptUpdateBar();renderSL();
            return;
          }
    ...
    (function(idx){
      del.onclick=function(e){
        e.stopPropagation();
        if(planMode)return;
        var rs=_ptGetRows();rs[idx]={subjId:rs[idx].subjId,text:'',status:null};_ptSave(rs);renderSL();
      };
    })(i);
    tr.appendChild(del);
    ...
    txt.textContent=hasText?row.text:'';
    (function(idx,tc){
      tc.addEventListener('click',function(e){
        e.stopPropagation();
        if(planMode)return; // 계획 모드 중 할일칸 클릭 차단
        _ptStartEdit(idx,tc);
      });
    })(i,txt);
    tr.appendChild(txt);
    ...
    (function(fromIdx,rowEl){
      ...
      function moveDrag(clientY){
        ...
        allRows.forEach(function(el,j){
          ...
          if(dist<minDist){minDist=dist;toIdx=j;insertBefore=(clientY<mid);}
        });
        if(toIdx>=0){
          var targetRow=allRows[toIdx];
          updateInsertLine(targetRow,insertBefore);
          // insertBefore 정보 저장
          rowEl._insertBefore=insertBefore;
          rowEl._toIdx=toIdx;
        }
      }
      ...
      function endDrag(){
        if(!dragging)return;
        dragging=false;
        rowEl.style.opacity='';rowEl.style.transform='';rowEl.style.boxShadow='';rowEl.style.zIndex='';rowEl.style.position='';
        if(insertLine){insertLine.remove();insertLine=null;}
        var tbody=document.getElementById('ptTbody');if(tbody)tbody.style.position='';

        var finalToIdx=rowEl._toIdx!=null?rowEl._toIdx:-1;
        var finalBefore=rowEl._insertBefore!=null?rowEl._insertBefore:true;
        delete rowEl._toIdx;delete rowEl._insertBefore;

        if(finalToIdx<0||finalToIdx===fromIdx)return;

        var rs=_ptGetRows();
        var allRows=Array.prototype.slice.call(document.querySelectorAll('#ptTbody .pt-row'));

        // 실제 인덱스 계산: 선이 가리키는 위치
        var item=rs.splice(fromIdx,1)[0];
        // toIdx는 DOM 행 인덱스 — fromIdx 제거 후 보정
        var adjTo=finalToIdx>fromIdx?finalToIdx-1:finalToIdx;
        var insertAt=finalBefore?adjTo:adjTo+1;
        insertAt=Math.max(0,Math.min(rs.length,insertAt));
...
function _ptStartEdit(idx,tc){
  ...
  tc.textContent='';tc.className='pt-row-txt';tc.appendChild(inp);
  inp.focus();if(inp.value)inp.select();
  var saved=false;
  function save(){
    if(saved)return;saved=true;
    var v=inp.value.trim();var rs2=_ptGetRows();rs2[idx].text=v;_ptSave(rs2);renderSL();
  }
  function saveAndNext(){
    ...
    setTimeout(function(){
      ...
      if(allRows[idx+1]){
        var nextTxt=allRows[idx+1].querySelector('.pt-row-txt');
        if(nextTxt)_ptStartEdit(idx+1,nextTxt);
      }
    },20);
  }
  inp.onblur=save;
  inp.onkeydown=function(ev){
    if(ev.key==='Enter'){ev.preventDefault();saveAndNext();}
    if(ev.key==='Escape'){saved=true;renderSL();}
  };
}
...
function clickSubj(id){
  ...
  selId=id;_ptSelId=id;_ptUpdateBar();
...
function _hexToRgb(hex){hex=(hex||'#888').replace('#','');if(hex.length===3)hex=hex[0]+hex[0]+hex[1]+hex[1]+hex[2]+hex[2];return parseInt(hex.substr(0,2),16)+','+parseInt(hex.substr(2,2),16)+','+parseInt(hex.substr(4,2),16);}
...
function addTodo(){var inp=document.getElementById('todoNewInput');var v=(inp&&inp.value||'').trim();if(!v||!todoOpenSubjId)return;var dk=today();if(!todos[dk])todos[dk]={};if(!todos[dk][todoOpenSubjId])todos[dk][todoOpenSubjId]=[];todos[dk][todoOpenSubjId].push({text:v,done:false});svTodos();renderTodoPanel();saveUserDataToFirebase();}
function toggleTodo(idx){var dk=today();if(!todoOpenSubjId||!todos[dk]||!todos[dk][todoOpenSubjId]||!todos[dk][todoOpenSubjId][idx])return;todos[dk][todoOpenSubjId][idx].done=!todos[dk][todoOpenSubjId][idx].done;svTodos();renderTodoPanel();saveUserDataToFirebase();}
function delTodo(idx){var dk=today();if(!todoOpenSubjId||!todos[dk]||!todos[dk][todoOpenSubjId])return;todos[dk][todoOpenSubjId].splice(idx,1);svTodos();renderTodoPanel();saveUserDataToFirebase();}
...
function addSubj(){var n=(document.getElementById('nSubjN')||{}).value;if(n)n=n.trim();if(!n){toast('과목 이름을 입력해주세요');return;}subjs.push({id:'s'+Date.now(),name:n,color:pickedC});sv();closeModal('addSM');renderScList();renderSL();renderHomeSubjGrid();renderTL();}
...
function updateStats(){
  var t=getTSecs(),h=Math.floor(t/3600),m=Math.floor((t%3600)/60);
  var el=document.getElementById('stTotal');if(el)el.textContent=h+'h'+(m>0?' '+m+'m':'');
  // 월평균: 이번 달 1일~오늘 총합 / 경과일수
  var el3=document.getElementById('stAvg');
  if(el3){
    var now=new Date(),days=now.getDate(),tot=0;
    for(var d=1;d<=days;d++)tot+=getSecsDate(now.getFullYear(),now.getMonth(),d);
    var avg=tot/days,ah=Math.floor(avg/3600),am=Math.floor((avg%3600)/60);
    el3.textContent=ah+'h'+(am>0?' '+am+'m':'');
  }
